...
public:
    ...
    MyQueue(MyQueue&& other) : tab(other.tab), msize(other.msize), head(other.head), tail(other.tail){
        other.tab=nullptr; //aby destruktor nie probowal zwolnic tablciy
        other.msize=1;
    ...
    void push(T&& item){
        assert(!full()); // Upewniamy się, że kolejka nie jest pełna
        tab[tail]= std::move(item); // Przenosimy element do kolejki
        tail=(tail+1)%msize;
    } // dodanie na koniec
